...
export function getCategoryIconUrl(
  ...
  // Diniy kategoriyalar uchun emoji ikonkalar
  const emojiMap: Record<string, string> = {
    // Diniy kategoriyalar
    aqida: "вЄпёЏ",
    aqoid: "вЄпёЏ",
    "aqoid-va-imon": "вЄпёЏ",
    imon: "рџ¤І",
    iymon: "рџ¤І",
    fiqh: "рџ“–",
    fiqih: "рџ“–",
  ...
  // 2. Diniy emoji kategoriyalarda tekshirish
  if (emojiMap[slugLower]) {
    return emojiMap[slugLower];
  }

  // 3. Local iconMap'da tekshirish (texnologiya kategoriyalari)
  if (iconMap[slugLower]) {
    return iconMap[slugLower];
  }

  // 4. API'dan kelgan boshqa iconlar
  if (apiIcon) {
    return getUploadUrl(apiIcon);
  }
